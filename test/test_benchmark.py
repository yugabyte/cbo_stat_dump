...
def parse_arguments():
    ...
    parser.add_argument('--target_is_yugabyte', action='store_true', help='Use this option if target database is YugabyteDB')
    ...
    parser.add_argument('--test_is_yugabyte', action='store_true', help='Use this option if test database is YugabyteDB')
    ...
    if args.colocation and not args.test_is_yugabyte:
        sys.stderr.writelines(f'Colocation is a YugabyteDB feature and not supported on Postgres\n')
        sys.stderr.writelines(f'--colocation can only be used with --test_is_yugabyte\n')
        sys.exit(1)
...
def export_query_plan(args, test_db_name, query_file, outdir):
    ...
    if args.enable_optimizer_statistics and not args.target_is_yugabyte:
...
def run_export_script(args, query_outdir, query_file_name_abs):
    ...
    if args.enable_optimizer_statistics and args.target_is_yugabyte:
        cmd.append('--enable_optimizer_statistics')
    if args.target_is_yugabyte:
        cmd.append('--yb_mode')
